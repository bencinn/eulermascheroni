import Mathlib.Analysis.SpecialFunctions.Log.Basic
import Mathlib.NumberTheory.Harmonic.EulerMascheroni
import Mathlib.Tactic

namespace EulerMascheroniInfiniteSum

open Filter

/- shift because fuck n=0 -/
noncomputable def eulerMascheroni_sum_inner (n : ℕ) :=
  1 / (↑(n + 1) : ℝ) - Real.log (↑(n + 2) / ↑(n + 1))

theorem eulerMascheroni_tsum :
  ∑' n, eulerMascheroni_sum_inner n = Real.eulerMascheroniConstant := by
    apply HasSum.tsum_eq
    unfold Real.eulerMascheroniConstant
    have h_nonneg : ∀ n, 0 ≤ eulerMascheroni_sum_inner n := by
      intro n
      unfold eulerMascheroni_sum_inner
      have h_pos : 0 < (↑(n + 2) : ℝ) / ↑(n + 1) := by positivity
      have h_log := Real.log_le_sub_one_of_pos h_pos
      field_simp at h_log
      simp at h_log
      push_cast at *
      have h_pos' : (0 : ℝ) < n + 1 := by positivity
      rw [mul_comm] at h_log
      rw [← le_div_iff₀ h_pos'] at h_log
      linarith
    have h_partial_sums : ∀ N, ∑ k ∈ Finset.range N, eulerMascheroni_sum_inner k = (harmonic N : ℝ) - Real.log (N + 1) := by
      intro N
      induction' N with n ih
      · simp [harmonic]
      · rw [Finset.sum_range_succ]
        rw [ih]
        unfold eulerMascheroni_sum_inner
        rw [harmonic_succ]
        push_cast
        have h_log : Real.log ((↑n + 2) / (↑n + 1)) = Real.log (↑n + 2) - Real.log (↑n + 1) := by
          exact Real.log_div (by positivity) (by positivity)
        rw [h_log]
        ring_nf
    have h_bound : ∀ N, ∑ k ∈ Finset.range N, eulerMascheroni_sum_inner k ≤ 1 := by
      intro N
      rw [h_partial_sums N]
      have h_lt := Real.eulerMascheroniSeq_lt_eulerMascheroniSeq' N 1
      rw [Real.eulerMascheroniSeq'_one] at h_lt
      unfold Real.eulerMascheroniSeq at h_lt
      linarith
    rw [Summable.hasSum_iff_tendsto_nat]
    simp_rw [h_partial_sums]
    exact Real.tendsto_eulerMascheroniSeq
    exact summable_of_sum_range_le h_nonneg h_bound
